...
import type { SheepSaveState } from "../systems/save-manager.ts";

const SHEEP_HP = 30;
const SHEEP_SPEED = 80; // px/sec — half player speed
...
export class Sheep extends Creature {
  ...
  protected override getDrops(): Item[] {
    return [{ ...ITEMS["mutton"] }, { ...ITEMS["wool"] }];
  }
  ...
  /**
   * Update AI behavior each frame. Called by the base Creature.onPreUpdate
  ...
  restoreState(state: SheepSaveState): void {
    this.hp = state.hp;
